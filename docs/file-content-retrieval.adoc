...
**Version:** 1.1 (Updated for maintenance best practices)
...
function fileContent_get(filePath: string): Promise<Result<string>> {
  ...
  // Future: if (filePath.startsWith('/PACS/')) { return fileContent_getPACS(filePath); }
  return fileContent_getRegular(filePath);
}
----

**Implementation:** link:../src/files/index.ts[`src/files/index.ts`] (lines ~562-567)
...
2. Check if path starts with `/PACS/` → delegate to `fileContent_getPACS()` (future)
...
This section provides a pattern for adding support for new file types (e.g., PACS files at `/PACS/*`).

=== Reference Implementation

**Study the pipeline handler** as a reference: link:../src/files/pipeline_content.ts[`pipeline_content.ts`]
...
Create `salsa/src/files/pacs_content.ts` following this **conceptual template**:
...
export async function fileContent_getPACS(filePath: string): Promise<Result<string>> {
  ...
  try {
    // 3. Query PACS files (investigate API method)
    // const pacsList = await client.getPACSFiles(/* params? */);

    // 4. Filter to find matching file
    // const matches = pacsList.getItems().filter(/* match logic */);

    // 5. Extract blob
    // const blob = await pacsFile.getFileBlob();

    // 6. Convert blob to string (copy pattern from pipeline_content.ts)
    // return Ok(content);
...
**Note:** This is a conceptual template. Actual implementation depends on ChRIS API structure for PACS files. See pipeline handler for blob conversion pattern.
...
[source,typescript]
----
import { fileContent_getPACS } from './pacs_content';  // Add import

export async function fileContent_get(filePath: string): Promise<Result<string>> {
  if (filePath.startsWith('/PIPELINES/')) {
    return fileContent_getPipeline(filePath);
  }
  if (filePath.startsWith('/PACS/')) {             // Add route
...
* **Router:** link:../src/files/index.ts[`src/files/index.ts`] (~562-567)
